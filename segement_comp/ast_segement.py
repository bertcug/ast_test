...
def segement_ast_similarity_process(vuln_name, patch_name, neo4jdb, worksheet, suffix_tree_obj):
    ...
    line = (vuln_name, patch_name,"success", report['distinct_type_and_const'],
            report['distinct_const_no_type'], ['distinct_type_no_const'],
            ['distinct_type_no_const'], cost)
...
if __name__ == "__main__":
    ...
    for row in ws.rows:
        try:
            segement_ast_similarity_process(row[0].value, row[1].value, neo4jdb, worksheet, suffix_tree_obj)
